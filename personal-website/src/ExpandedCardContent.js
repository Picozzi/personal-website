...
import { expandedVariants, contentBlockVariants, mainImageVariants } from './variants';
...
const ExpandedCard = ({data, onClick}) => {
    ...
    return (
        ...
        <ExpandedCardContent color={data.jobColor}>
            ...
            <AnimatedContentBlock>
            {data.jobNews != null ? 
            <h3>In The News</h3> : null 
            }
            {
                data.jobNews != null ?
            (data.jobNews).map((point, key) => (
                <h2 key={key}><a href={point[0]} target="_blank" rel="noopener noreferrer">{point[1]}</a></h2>
            )) : null }          
            </AnimatedContentBlock>
        </ExpandedCardContent>
        ...
        <Close color={data.jobColor}>
            ...
            {IsPhoneSmall ?
                <Icon name='close' size="large"/> : <Icon name='close' size="tiny"/> }
            </a>
        </Close>
        </AnimatedContentBlock>
